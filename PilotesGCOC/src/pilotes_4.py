...
def calcular_pilote(D, L, df, zw, fS_val, sigma_tope_mpa):
    ...
    for idx, row in df.iterrows():
        ...
        if overlap_h > 0 and espesor_bulbo > 0:
            ...
            # --- VOLVEMOS A LA REGLA MATEMÁTICA PURA SEGÚN GCOC ---
...
def generar_word_pilotes(df_estratos, fig_tens, df_pivot_geo, df_pivot_final, df_res, fS_val, situacion, D_array, zw_val, sigma_tope):
    ...
    p_bases.add_run(f'Si el bulbo de influencia 3D supera la profundidad explorada, se adopta la hipótesis de prolongación indefinida del estrato base.')
    ...
    p_metodo.add_run('Cálculo de resistencia equivalente ponderada en función de los espesores interceptados evaluando un bulbo de rotura extendido 6D por encima y 3D por debajo de la base del pilote. Se aplica factor de reducción por efecto de escala (fD).\n\n')
