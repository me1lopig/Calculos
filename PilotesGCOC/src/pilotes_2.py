...
with tab_datos:
    ...
    if (df_edit.loc[mascara_corto_plazo, "phi (grados)"] != 0.0).any():
        ...
        st.info("💡 **Ajuste automático:** El sistema ha fijado el valor de φ a 0º en los estratos a Corto Plazo para evitar confusiones en el modelo de Tresca.")
...
if st.session_state.calculado:
    ...
    if not df_res.empty:
        ...
        with tab_auditoria:
            ...
            # --- MODIFICADO: Formateo estricto a 2 decimales en el menú desplegable ---
            ...
            if not res_auditoria.empty:
                ...
                # --- MODIFICADO: Añadido .hide(axis="index") a la tabla del fuste ---
                ...
                # --- MODIFICADO: Mantengo hide(axis="index") aquí también ---
                ...
                # --- MODIFICADO: El gráfico ahora se renderiza ocupando todo el ancho inferior ---
