...
# ══════════════════════════════════════════════════════════════════════════
# INICIALIZACIÓN DE LA TABLA BASE (TEXTOS LIMPIOS)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
tab_datos, tab_tensiones, tab_matriz_punta, tab_matriz_fuste, tab_matriz_total, tab_matriz_tope, tab_graficas, tab_auditoria, tab_formulacion = st.tabs([
    "📋 1. Estratigrafía", "🌊 2. Tensiones", "🔻 3. Punta", "🟫 4. Fuste", "🌍 5. Total", "🛑 6. Tope Estruct.", "📈 7. Gráficas", "🔍 8. Auditoría", "📖 9. Formulación"
])
...
with tab_datos:
    ...
    # --- AUTOCORRECCIÓN INTELIGENTE DE PHI ---
    # Si se selecciona "Corto Plazo", forzamos el phi a 0 de forma estricta
    ...
    if (df_edit.loc[mascara_corto_plazo, "phi (grados)"] != 0.0).any():
        ...
        st.session_state.df_base = df_edit.copy() # Sincronizamos para que la UI lo muestre
        st.toast("💡 φ ajustado automáticamente a 0º", icon="⚙️")
        st.rerun() # Forzamos un refresco inmediato de la pantalla para limpiar la celda visualmente
...
if st.session_state.calculado:
    ...
    with tab_datos:
        ...
        for idx, row_est in df_edit.iterrows():
            ...
            datos_unitarios.append({
                ...
                "Resist. Punta Base, q_p (kPa) | fD=1": "{:.0f}".format(qp_base)
            })
    ...
    if not df_res.empty:
        ...
        with tab_matriz_total:
            ...
            st.markdown("---")
            st.subheader("📈 Curvas de Capacidad Portante Global")
            df_plot_total = df_res.copy()
            df_plot_total["Diámetro"] = df_plot_total["D"].apply(lambda x: f"Ø {x:.2f} m")
            fig_total = px.line(df_plot_total, x="L", y="Q_adm_geo (kN)", color="Diámetro", markers=True,
                                title="Capacidad Geotécnica Total vs. Longitud")
            fig_total.update_layout(xaxis_title="Longitud L (m)", yaxis_title="Carga Admisible (kN)", hovermode="x unified")
        ...
        with tab_graficas:
            ...
            D_target = st.selectbox("Selecciona un Diámetro para evaluar el límite estructural:", D_arr, format_func=lambda x: f"{x:.2f}")
        ...
        with tab_auditoria:
            ...
            d_aud = col_aud1.selectbox("Selecciona Diámetro Ø (m):", D_arr, format_func=lambda x: f"{x:.2f}", key="d_aud")
            l_aud = col_aud2.selectbox("Selecciona Longitud L (m):", L_arr, format_func=lambda x: f"{x:.2f}", key="l_aud")
            ...
            if not res_auditoria.empty:
                ...
                st.markdown(f"#### 🧅 Composición del Bulbo (6D/3D)")
                ...
                st.markdown("---")
                st.markdown(f"#### 📐 Esquema Gráfico del Bulbo")
                ...
                fig_bulbo.add_shape(type="rect", x0=-D_val/2, x1=D_val/2, y0=0, y1=L_val, fillcolor="darkgray", line=dict(color="black", width=2))
                ...
                rango_sup = max(0, z_sup - 3*D_val)
                rango_inf = min(z_max_total, z_inf + 3*D_val)
                
                fig_bulbo.update_yaxes(autorange="reversed", title="Profundidad (m)", range=[rango_inf, rango_sup], showgrid=True)
                fig_bulbo.update_xaxes(showticklabels=False, range=[-D_val*4, D_val*4], showgrid=False)
                fig_bulbo.update_layout(height=450, margin=dict(l=20, r=20, t=20, b=20), paper_bgcolor="white", plot_bgcolor="white")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # GENERACIÓN Y DESCARGA DE INFORME (AHORA EN LA BARRA LATERAL)
    # ══════════════════════════════════════════════════════════════════════════
    st.sidebar.markdown("---")
