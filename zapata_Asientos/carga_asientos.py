...
from scipy.optimize import brentq  # SOLUCIONADOR NUMÉRICO
...
def z_influencia_mejorada(p, B, L, df, NF, z_max_usuario):
    et = float(pd.to_numeric(df["Espesor (m)"]).sum())
    limite_geometrico = 3.0 * B  
    ...
    return min(z_ec7, limite_geometrico, et, z_max_usuario)

# ══════════════════════════════════════════════════════════════════════════
# SOLVER INVERSO: ENCONTRAR PRESION ADMISIBLE
# ══════════════════════════════════════════════════════════════════════════
def encontrar_presion_admisible(metodo_func, s_max_objetivo, B, L, df_terreno, NF, z_max_usuario, dz_sub=None):
    def diferencia_asiento(p_prueba):
        zi_actual = z_influencia_mejorada(p_prueba, B, L, df_terreno, NF, z_max_usuario)
        if dz_sub is None:
    ...
    try:
        ...
        zi_final = z_influencia_mejorada(p_optima, B, L, df_terreno, NF, z_max_usuario)
        if dz_sub is None:
        ...
        st.error("No se pudo encontrar una presión que genere ese asiento en el rango evaluado.")
...
def generar_word(B, L, s_max_mm, p_st, p_ec, NF, z_max, zi_final, df_terreno,
    ...
    data = [
        ...
        ('Profundidad de influencia adoptada (zi)', f'{zi_final:.2f} m'),
        ('Profundidad de bulbo manual seleccionada', f'{z_max:.1f} m'),
        ('Límite Geométrico aplicado', f'3B = {3*B:.1f} m')
    ]
...
st.sidebar.info(f"**n = L/B:** {L/B:.2f} | **Límite 3B:** {3*B:.1f} m")
...
z_max_user = st.sidebar.number_input(
    ...
    help="Fuerza el corte del bulbo a esta profundidad si es menor que los criterios normativos."
)
...
if st.sidebar.button("🚀 Calcular Presión Admisible", type="primary", use_container_width=True):
    with st.spinner("Iterando presiones admisibles..."):
        p_st, tot_st, df_st, zi_st = encontrar_presion_admisible(
            calcular_steinbrenner, s_max_m, B, L, st.session_state.df_terreno, NF, z_max_user
        )
        
        p_ec, tot_ec, df_ec, zi_ec = encontrar_presion_admisible(
            calcular_ec68, s_max_m, B, L, st.session_state.df_terreno, NF, z_max_user, dz_sub
        )
...
if st.session_state.calculo_realizado:
    ...
    word_buf = generar_word(
        ...
        st.session_state.zi_final, st.session_state.df_terreno,
...
if modo == "🧮 Panel de Resultados":
    ...
    st.markdown(
        ...
        "el tamaño del bulbo de presiones se recalcula dinámicamente."
    )
    
    st.markdown("---")
    # --- Parte 2: Fórmulas Originales ---
    ...
    st.markdown(
        "Para la integración del asiento, el programa adopta como límite final el menor de estos 4 valores:\n"
        "1. **Límite Tensional (EC7):** Profundidad donde el incremento $\Delta\sigma_z \leq 0.20\,\sigma'_{v0}$\n"
        "2. **Límite Geométrico:** $3 x B$ (evita bulbos irreales en cimentaciones pequeñas)\n"
        "3. **Límite Físico:** El espesor total de la estratigrafía introducida\n"
        "4. **Límite Manual:** El valor forzado por el usuario como 'Profundidad de bulbo manual'"
    )