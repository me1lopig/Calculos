    ...
    for i, column in enumerate(df.columns):
        ...
        for paragraph in hdr_cells[i].paragraphs:
            ...
            for run in paragraph.runs:
                run.font.bold = True
                run.font.size = Pt(8) 
                # CORRECCIÓN: Letra azul oscuro para que contraste con fondo claro
...
def generar_word(B, L, p, NF, z_max, zi, df_terreno,
    ...
    # Configurar márgenes elegantes y pie de página
    for sec in doc.sections:
        ...
        footer_para.text = f"Memoria de Cálculo de Asientos — Generado automáticamente el {fecha}"
    ...
    r_title = p_title.add_run('MEMORIA DE CÁLCULO DE ASIENTOS')
    r_title.bold = True
    ...
    data = [
        ...
        ('Profundidad de influencia (z_i según EC7)', f'{zi:.2f} m'),
        ('Profundidad máxima evaluada (z_max)', f'{z_max:.1f} m'),
        ('Criterio tensional aplicado', 'Tensiones bajo el centro (Superposición 4x cuadrantes)')
    ]
    ...
    doc.add_heading('2. Cálculos Intermedios', level=1)
    p_info = doc.add_paragraph('A continuación se desglosan los parámetros de integración y cálculo tensional, estrato por estrato, para ambos métodos.')
    p_info.runs[0].font.color.rgb = RGBColor(89, 89, 89)
    
    _add_styled_table(doc, df_st, '2.1 Método de Steinbrenner (Factores de Influencia analíticos)')
    doc.add_page_break()
    _add_styled_table(doc, df_ec, '2.2 Método Elástico (Tensiones de Holl y deformación unitaria media)')
    doc.add_page_break()

    # ── 3. CÁLCULOS DE ASIENTOS ──────────────────────────────────────────
    doc.add_heading('3. Cálculos de Asientos (Resultados Finales)', level=1)
    
    # Bloque destacado para los resultados finales
    ...
    r1a = p1.add_run('Asiento Método Steinbrenner\n')
    # CORRECCIÓN: Letras oscuras
    r1a.font.size = Pt(11); r1a.font.color.rgb = RGBColor(89, 89, 89); r1a.bold = True
    r1b = p1.add_run(f'{tot_st*1000:.3f} mm')
    r1b.font.size = Pt(16); r1b.bold = True; r1b.font.color.rgb = RGBColor(23, 54, 93)
    
    p2 = c2.paragraphs[0]
    p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r2a = p2.add_run('Asiento Ecuación Elástica\n')
    # CORRECCIÓN: Letras oscuras
    r2a.font.size = Pt(11); r2a.font.color.rgb = RGBColor(89, 89, 89); r2a.bold = True
    r2b = p2.add_run(f'{tot_ec*1000:.3f} mm')
    r2b.font.size = Pt(16); r2b.bold = True; r2b.font.color.rgb = RGBColor(33, 115, 70)

    doc.add_paragraph()
    doc.add_paragraph()
    
    # Tabla resumen comparativa muy limpia
    df_comp = pd.DataFrame({
        "Capa": df_st["Capa"],
        "Δs Steinbrenner [mm]": df_st["Δs [mm]"],
        "Δs Ec. Elástica [mm]": df_ec["Δs [mm]"].values,
    })
    _add_styled_table(doc, df_comp, '3.1 Resumen Comparativo de Asientos por Estrato')

    # ── 4. GRÁFICA DEL BULBO DE TENSIONES ────────────────────────────────
    doc.add_page_break()
    doc.add_heading('4. Gráfica del Bulbo de Tensiones', level=1)
    p_fig = doc.add_paragraph()
    p_fig.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_fig = p_fig.add_run()
    r_fig.add_picture(fig_bulbo_bytes, width=Cm(14))
    
    nota = doc.add_paragraph(f'Evolución en profundidad de las tensiones bajo el centro geométrico de la zapata (p={p} kPa, B={B} m, L={L} m). La línea discontinua naranja marca el criterio de la profundidad de influencia de EC7.')
